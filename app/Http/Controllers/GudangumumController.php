...
{
    protected $domain = env('API_BACKEND', 'http://localhost/api_backend_test/');
    

    public function __construct(){
        if (str_contains($_SERVER['SERVER_NAME'], '136.198.117.') || str_contains($_SERVER['SERVER_NAME'], 'localhost'))
        {
            $this->domain = env('API_BACKEND_TEST', 'http://localhost/api_backend_test/');
        }
    }
    //  **
    //  index
    ...
    public function loaddata(Request $request,$valjmlhal=1,$jumlahDataPerHalaman=10)
    {
        if($request->ajax())
        {
            ...
            $data = Http::get($this->domain.'json_gudangumum.php',[
                'valstdate' => $stdate,
                'valendate' => $endate,
                'valpartno' => $partno,
                'start' => $awalData,
    ...
    public function download(Request $request)
    {
        ...
        $datas = Http::get($this->domain.'json_gudangumum.php',[
                    'valstdate' => $stdate,
                    'valednate' => $endate,
                    'valpartno' => $partno
                ]);
